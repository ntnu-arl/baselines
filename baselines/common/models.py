...
def mlp_rmf_actor():
    ...
    def network_fn(input_shape):
        ...
        #h3 = tf.keras.layers.Dense(units=16, kernel_initializer=ortho_init(np.sqrt(2)), name='mlp_fc3',activation='relu')(h2)                                                                        
...
def mlp_rmf_critic():
    ...
    def network_fn(input_shape):
        ...
        h2 = tf.keras.layers.Dense(units=64, kernel_initializer=ortho_init(np.sqrt(2)),
                                    name='mlp_fc2', activation='relu')(h1)
        #h3 = tf.keras.layers.Dense(units=32, kernel_initializer=ortho_init(np.sqrt(2)), name='mlp_fc3',activation='relu')(h2)                                     
        network = tf.keras.Model(inputs=[x_input], outputs=[h2])
        return network
